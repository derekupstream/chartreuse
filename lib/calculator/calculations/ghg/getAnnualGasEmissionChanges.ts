...
import { NO_MATERIAL_ID } from '../../constants/materials';
import {
...
import { getReusableItemTransportationGHG } from './getTransportationGHG';
...
export function getAnnualGasEmissionChanges(project: ProjectInventory): AnnualGasEmissionChanges {
  const lineItems = project.singleUseItems.map(lineItem =>
    getLineItemGasEmissions({ lineItem, frequency: lineItem.frequency, isEventProject: project.isEventProject })
  );
  ...
  const dishwashing = getDishwasherGasEmissions(project.dishwashers, project.racksUsedForEventProjects);
  const reusableItemtransportationGHG = getReusableItemTransportationGHG(project);
  ...
  return {
    ...
    total: getChangeSummaryRowRounded(
      ...
      landfillWaste.forecast + shippingBox.forecast + dishwashing.forecast + reusableItemtransportationGHG,
      2
    )
  };
}
...
export function getDishwasherGasEmissions(
...
export function getLineItemGasEmissions({
  ...
  // Columns: X, Y
  const annualBoxWeight = isEventProject
