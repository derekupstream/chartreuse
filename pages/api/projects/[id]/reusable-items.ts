...
export default async function handler(
  ...
  if (req.method === "GET") {
    const lineItems =
      await prisma.reusableLineItem.findMany<Prisma.ReusableLineItemFindManyArgs>(
        {
          where: {
            projectId,
          },
    ...
    try {
      const lineItem =
        await prisma.reusableLineItem.create<Prisma.ReusableLineItemCreateArgs>(
          {
            data: req.body,
          }
        );
    ...
    try {
      await prisma.reusableLineItem.delete<Prisma.ReusableLineItemDeleteArgs>({
        where: {
          id: projectId,
        },
      });
