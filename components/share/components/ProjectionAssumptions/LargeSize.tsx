export function LargeSize() {
  return (
    <>
      <p className='c1'>
        <span className='c2 c14'>Reuse purchasing:</span>
      </p>
      <p className='c1 c15'>
        <span className='c4 c2'></span>
      </p>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-0 start'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c2'>
            Initial reuse purchasing quantity references the industry-standard 3:1 ratio of reusable items to daily
            disposable items
          </span>
          <sup className='c2'>
            <a href='#ftnt1' id='ftnt_ref1'>
              [1]
            </a>
          </sup>
          <span className='c4 c2'>. </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            This ratio ensures optimal product lifespan and inventory volume by allowing &#8531; of reusables to be in
            use, &#8531; to be on drying racks, and &#8531; to be washed.
          </span>
        </li>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            For example, if a 600 customer restaurant sold 600 meals served on disposable plates per day, that
            restaurant should adhere to the 3:1 ratio by initially investing in 1800 reusable plates.{' '}
          </span>
        </li>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>This 3:1 ratio may also be referred to as a &ldquo;circulation rate&rdquo; of 3</span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-0'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>
            The lifespan of reusable materials is reflected by the number of washes they are able to withstand
            throughout their lifetime.{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            This value varies across materials, increasing with the durability of a product material.
          </span>
        </li>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            The following reusable materials were used in the 600 customer restaurant, primarily referencing
            retailer/manufacturer data for their lifespan:
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c2'>SAN plastic- 1,000 washes</span>
          <sup className='c2'>
            <a href='#ftnt2' id='ftnt_ref2'>
              [2]
            </a>
          </sup>
          <span className='c4 c2'>&nbsp;</span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c2'>Melamine- 1,217 washes</span>
          <sup className='c2'>
            <a href='#ftnt3' id='ftnt_ref3'>
              [3]
            </a>
          </sup>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c2'>Stainless steel- 10,950 washes</span>
          <sup className='c2'>
            <a href='#ftnt4' id='ftnt_ref4'>
              [4]
            </a>
          </sup>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-1'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c2'>
            Industry material lifespan data is used to calculate annual repurchase quantity accounting for
            damage/material wear.{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-0'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>
            Recovery rate is expressed as the percentage of materials properly returned by customers to be washed by the
            restaurant.{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            The total annual repurchase quantity of reusable materials will consider annual loss rate ((1- Recovery
            rate)*100%)). &nbsp;{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_axi9p0iwik24-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            The templates assumed the recovery rate would increase minimally as the restaurant size decreased.
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-1'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            For the use case of a large, franchised on-site dining restaurant with a 100% switch to reuse, the template
            used a recovery rate of 75%.{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>
            This value is likely an overestimate for this closed-loop system, but accommodates for a larger loss rate
            while customers transition to reuse
          </span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>
            Note that recovery rates may be increased with proper signage and education surrounding reuse{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-0'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>
            Total annual reusable repurchase quantity = RR-based repurchase quantity + damage/wear repurchase quantity
          </span>
        </li>
      </ul>
      <p className='c1 c15'>
        <span className='c4 c2'></span>
      </p>
      <p className='c1'>
        <span className='c2 c18'>Single-use purchasing: </span>
        <span className='c4 c2'>
          <br />
        </span>
      </p>
      <ul className='c8 lst-kix_sg712et45snh-0 start'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>
            The annual single-use purchasing quantity uses a place-setting per customer approach{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_50t07fy6fdzp-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            For a large restaurant serving approximately 600 customers per day, it is assumed each customer consumes a
            &ldquo;place setting&rdquo; of disposable materials consisting of:
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_sg712et45snh-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>0.5 10 3/8&quot;-Dinner Plate- Molded Fiber</span>
        </li>
      </ul>
      <ul className='c8 lst-kix_ua8rnhzgoxxt-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>0.5 Paper Food Tray (3lb)- Polycoated Paper</span>
        </li>
      </ul>
      <ul className='c8 lst-kix_sg712et45snh-2'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>1 Fork (heavy weight)- PS </span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>1 Spoon (heavy weight)- PS</span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>1 Knife (heavy weight)- PS</span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>0.33 8 oz Clear Cold Cup- PET </span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>0.33 12 oz Clear Cold Cup- PET </span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>0.33 16 oz Clear Cold Cup- PET</span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>2 Ketchup (.32 oz ea)- LDPE &nbsp;</span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>2 Mustard (.2 oz ea)- LDPE &nbsp;</span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>5 Single-ply Napkin</span>
        </li>
      </ul>
      <ul className='c8 lst-kix_sg712et45snh-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c2'>
            The place setting disposable materials and their reusable substitute were established through both previous
            Upstream beta tester data and restaurant single-use purchasing data from Rethink Disposable case studies.
          </span>
          <sup className='c2'>
            <a href='#ftnt5' id='ftnt_ref5'>
              [5]
            </a>
          </sup>
        </li>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            The templates assume menu offerings increase with the size of restaurant, particularly in for a large,
            franchised restaurant with multiple locations.{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_sg712et45snh-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c2 c4'>
            Subsequently, the place setting per customer for larger restaurants includes more size and material
            single-use options.{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-0'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>
            Assumes 365 days of operating per year for consistency across variables and comparability across templates
          </span>
        </li>
        <li className='c1 c13 li-bullet-0'>
          <span className='c2'>
            All pricing inputs assumed businesses would buy in bulk, and therefore receive the largest possible quantity
            discount
          </span>
          <sup className='c2'>
            <a href='#ftnt6' id='ftnt_ref6'>
              [6]
            </a>
          </sup>
        </li>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>
            Assumes on-site dining restaurant is making the switch from 100% disposable products to 100% reuse, with the
            exception of foodware accessories like straws or napkins{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_lk9e1l4y6pjk-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            Therefore, the templates project a 0% repurchase rate of disposables in the forecast scenario with the
            exception of accessories
          </span>
        </li>
      </ul>
      <p className='c1 c15'>
        <span className='c4 c2'></span>
      </p>
      <p className='c1'>
        <span className='c14 c2'>Additional costs: </span>
      </p>
      <p className='c1 c15'>
        <span className='c4 c2'></span>
      </p>
      <ul className='c8 lst-kix_aus0wjp6e4jb-0 start'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>Dishwashing: </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>On-site dishwashing was assumed for all restaurant templates.</span>
        </li>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            The necessary number of racks per day was estimated using the following assumptions:{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c2'>One standard commercial dishwashing rack holds 25 dishes, or 5 place-settings</span>
          <sup className='c2'>
            <a href='#ftnt7' id='ftnt_ref7'>
              [7]
            </a>
          </sup>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>
            The daily number of disposable products used = the daily number of washes required for reuse
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-3 start'>
        <li className='c1 c6 li-bullet-0'>
          <span className='c4 c2'>Ex. 1 disposable paper bowl = 1 reusable ceramic bowl used and washed</span>
        </li>
      </ul>
      <ul className='c8 lst-kix_q0wckymscz7h-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c2'>
            The type of dishwashing equipment was determined by the size of the venue. For small restaurants (150 daily
            customers), an under-counter dishwasher is recommended. For mid to large restaurants (400 daily customers),
            a stationary pull-down dishwasher. For very large restaurants/cafes/chains (600+ daily customers), a
            single-tank conveyor.
          </span>
          <sup className='c2'>
            <a href='#ftnt8' id='ftnt_ref8'>
              [8]
            </a>
          </sup>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-1'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            Referencing Upstream beta tester engagements of similar sizes, the templates assumed dishwashers were
            high-temperature, Energy star certified, and electric-powered. &nbsp;
          </span>
        </li>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            Because the baseline scenario assumes the restaurant is fully disposable, the template entry is 0 racks/day
            for reusables{' '}
          </span>
        </li>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            The template assumed this restaurant was subject to paying California utility rates (as the US state with
            the largest population).{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-0'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>Dishwashing labor:</span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>Dishwashing labor was calculated on a per-cycle basis </span>
        </li>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>
            Labor times were projected as they related to the processes of scraping/rinsing, loading, and unloading per
            cycle.
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>
            Note that dishwashing labor estimates only apply to the time spent scraping, rinsing, loading and unloading
            average daily reusables. Dishwashing labor estimates do not include cycle times as they assume staff members
            are scraping, loading, and unloading as each cycle runs. This estimate is meant to be considered in addition
            to other bussing and back of house work.{' '}
          </span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>
            It was estimated that one staff member would take approximately 50 seconds per cycle for each of the three
            processes.
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-3 start'>
        <li className='c1 c6 li-bullet-0'>
          <span className='c4 c2'>
            Businesses may find necessary labor time to be lower than what is projected in these templates{' '}
          </span>
        </li>
        <li className='c1 c6 li-bullet-0'>
          <span className='c2'>High-temperature dishwashers omitted rinsing from labor estimates</span>
          <sup className='c2'>
            <a href='#ftnt9' id='ftnt_ref9'>
              [9]
            </a>
          </sup>
          <span className='c4 c2'>&nbsp;</span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-1'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>An hourly rate of $20 was established for dishwashing staff</span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-0'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>Waste hauling: </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c2'>
            The template assumed restaurants were subject to paying California waste hauling rates (as the US state with
            the largest population)
          </span>
          <sup className='c2'>
            <a href='#ftnt10' id='ftnt_ref10'>
              [10]
            </a>
          </sup>
          <span className='c4 c2'>. </span>
        </li>
        <li className='c1 c7 li-bullet-0'>
          <span className='c2'>
            The template referenced Waste Management&rsquo;s waste hauling rates (as the largest waste hauling company
            in the US)
          </span>
          <sup className='c2'>
            <a href='#ftnt11' id='ftnt_ref11'>
              [11]
            </a>
          </sup>
          <span className='c4 c2'>. </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_fcukx7ocinev-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>
            Waste hauling fees per cubic yard were assumed to decrease slightly as restaurant size increased, because
            larger restaurants likely have more waste overall and therefore larger dumpsters.
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-2'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c2'>
            The template used Waste Management&rsquo;s quote of $58.97 per cubic yard of trash (at a total monthly rate
            of $471.72 for a 2-yd dumpster with 4 pickups per month),
          </span>
          <sup className='c2'>
            <a href='#ftnt12' id='ftnt_ref12'>
              [12]
            </a>
          </sup>
          <span className='c2'>&nbsp;with the assumption a cubic yard was approximately 275 lb of trash.</span>
          <sup className='c2'>
            <a href='#ftnt13' id='ftnt_ref13'>
              [13]
            </a>
          </sup>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-0'>
        <li className='c1 c13 li-bullet-0'>
          <span className='c4 c2'>Additional one-time reuse expenses: </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-1 start'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>Bus tubs/return stations </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>
            This template assumed each bus tub could fit approximately 5 place settings (25 dishes), which would be
            emptied approximately twice an hour throughout a 12 hour day.{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-3 start'>
        <li className='c1 c6 li-bullet-0'>
          <span className='c2'>
            For example, a 600 customer restaurant was found to require 5 return stations at $19.86/bus tub for a total
            one-time cost of $99.30.{' '}
          </span>
          <sup className='c2'>
            <a href='#ftnt14' id='ftnt_ref14'>
              [14]
            </a>
          </sup>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-1'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>Commercial dish drying racks:</span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c2'>Each place setting was assumed to require approximately 74.11 in</span>
          <span className='c2 c17'>2 </span>
          <sup className='c2'>
            <a href='#ftnt15' id='ftnt_ref15'>
              [15]
            </a>
          </sup>
          <span className='c4 c2'>
            , and the total surface area required for drying was calculated under the assumption of the 3:1 ratio that
            &#8531; of total reusable dishes would require space for drying at a time.
          </span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c2'>
            Assumes a commercial drying rack has four 24&rdquo; x 60&rdquo; shelves and costs $472.49.
          </span>
          <sup className='c2'>
            <a href='#ftnt16' id='ftnt_ref16'>
              [16]
            </a>
          </sup>
          <span className='c4 c2'>&nbsp; </span>
        </li>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>
            The total required drying surface area was used to calculate the number of commercial drying racks needed.{' '}
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-1'>
        <li className='c1 c7 li-bullet-0'>
          <span className='c4 c2'>Commercial dishwasher: </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-2 start'>
        <li className='c1 c11 li-bullet-0'>
          <span className='c4 c2'>
            Because these templates operate under the assumption each restaurant is switching from 100% disposable to
            100% reuse, the one-time costs include a commercial dishwasher.
          </span>
        </li>
      </ul>
      <ul className='c8 lst-kix_aus0wjp6e4jb-3 start'>
        <li className='c1 c6 li-bullet-0'>
          <span className='c2'>
            For a 600 customer restaurant, a high temperature, Energy star certified, electric single-tank conveyor
            dishwasher of $12,099
          </span>
          <sup className='c2'>
            <a href='#ftnt17' id='ftnt_ref17'>
              [17]
            </a>
          </sup>
          <span className='c2'>&nbsp;was included as a one-time cost. </span>
        </li>
      </ul>
      <hr className='c12' />
      <div>
        <p className='c9'>
          <a href='#ftnt_ref1' id='ftnt1'>
            [1]
          </a>
          <span className='c10'>&nbsp; </span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://resources.centralrestaurant.com/dinnerware-drinkware-flatware-ordering-estimates/&amp;sa=D&amp;source=editors&amp;ust=1714083839904148&amp;usg=AOvVaw3txTWh1DDPXXlG5hvbAOHp'
            >
              3:1 on-site buying guide{' '}
            </a>
          </span>
        </p>
        <p className='c9'>
          <span className='c10'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.getserveware.com/implementing-eco-takeouts-program-colleges-universities/&amp;sa=D&amp;source=editors&amp;ust=1714083839904884&amp;usg=AOvVaw1cY4Fqz09feQcp7eih3OLl'
            >
              3:1 reuse standard{' '}
            </a>
          </span>
        </p>
        <p className='c9'>
          <span className='c5'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref2' id='ftnt2'>
            [2]
          </a>
          <span className='c10'>&nbsp; </span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.getserveware.com/which-plastic-best-for-foodservice-drinkware-tritan-vs-san-vs-pc/&amp;sa=D&amp;source=editors&amp;ust=1714083839905641&amp;usg=AOvVaw1VSih4IPWXzKhUQJGIAmlb'
            >
              SAN plastic commercial lifespan{' '}
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref3' id='ftnt3'>
            [3]
          </a>
          <span className='c10'>&nbsp; </span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.getserveware.com/lifespan-of-melamine-plate/&amp;sa=D&amp;source=editors&amp;ust=1714083839906184&amp;usg=AOvVaw1gjzomAwoHdruTaZMkfibE'
            >
              Melamine commercial lifespan
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref4' id='ftnt4'>
            [4]
          </a>
          <span className='c10'>&nbsp; </span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://stainless-steel-world.net/stainless-steels-and-co2-industry-emissions-and-related-data/&amp;sa=D&amp;source=editors&amp;ust=1714083839906729&amp;usg=AOvVaw0KwLtg99xwuh7mbH4neTXH'
            >
              Stainless steel commercial lifespan
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref5' id='ftnt5'>
            [5]
          </a>
          <span>&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://cleanwater.org/author/rethink-disposable-case-studies&amp;sa=D&amp;source=editors&amp;ust=1714083839913625&amp;usg=AOvVaw3pszmC42zmqseRjfpuiHxB'
            >
              ReThink Disposable case studies{' '}
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref6' id='ftnt6'>
            [6]
          </a>
          <span>&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.webstaurantstore.com/&amp;sa=D&amp;source=editors&amp;ust=1714083839914053&amp;usg=AOvVaw196-jGIBkmpQ3gZRUCJRw_'
            >
              Commercial foodware pricing{' '}
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref7' id='ftnt7'>
            [7]
          </a>
          <span className='c10'>&nbsp; </span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.webstaurantstore.com/noble-products-full-size-all-purpose-peg-rack-with-open-sides/274RKOPAPPEG.html?utm_source%3Dgoogle%26utm_medium%3Dcpc%26utm_campaign%3DGoogleShopping%26gclid%3DCj0KCQiAnrOtBhDIARIsAFsSe51kbMXNAWYrNm58HEMtUdDg1qyw5TN7sF6Z5CHaGEHWbZoh6VrXl9MaAsYfEALw_wcB&amp;sa=D&amp;source=editors&amp;ust=1714083839907450&amp;usg=AOvVaw00CvxI_3vScgLUe6PeWKZs'
            >
              Dishwashing rack measurement
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref8' id='ftnt8'>
            [8]
          </a>
          <span className='c10'>&nbsp; </span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://chemmarkinc.com/commercial-dishwasher-types-explained/&amp;sa=D&amp;source=editors&amp;ust=1714083839913155&amp;usg=AOvVaw0SFOcb2hZ7_nDFh1dFfNee'
            >
              Dishwasher buying guide and rack
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref9' id='ftnt9'>
            [9]
          </a>
          <span className='c10'>&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://cdn2.webdamdb.com/md_EPytY9sed86.jpg.pdf?1626175649&amp;sa=D&amp;source=editors&amp;ust=1714083839907994&amp;usg=AOvVaw19mz78TsOhklTG0f5ltwQq'
            >
              Dish room operating guidelines
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref10' id='ftnt10'>
            [10]
          </a>
          <span className='c10'>&nbsp; </span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://worldpopulationreview.com/states&amp;sa=D&amp;source=editors&amp;ust=1714083839909015&amp;usg=AOvVaw0Sxa0QaIKUzI9GU-GbEuob'
            >
              US state populations
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref11' id='ftnt11'>
            [11]
          </a>
          <span className='c10'>&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.statista.com/statistics/1046508/us-canada-ranking-waste-haulers-by-revenue/&amp;sa=D&amp;source=editors&amp;ust=1714083839908526&amp;usg=AOvVaw2oQTCBww804VlYmeA-DCDp'
            >
              US waste hauler ranking
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref12' id='ftnt12'>
            [12]
          </a>
          <span className='c10'>&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.wm.com/us/en/business/business-waste-recycling-pickup&amp;sa=D&amp;source=editors&amp;ust=1714083839909532&amp;usg=AOvVaw0LEAvtkLAaMkL2zZmaVyBZ'
            >
              WM regional quotes
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref13' id='ftnt13'>
            [13]
          </a>
          <span className='c10'>&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://vinedisposal.com/debris-weight-guide.html&amp;sa=D&amp;source=editors&amp;ust=1714083839910014&amp;usg=AOvVaw11OrPNM0Ly_onFJl3a3wOf'
            >
              Trash cubic yard mass equivalent
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref14' id='ftnt14'>
            [14]
          </a>
          <span className='c10'>&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.katom.com/144-21157CBR110.html&amp;sa=D&amp;source=editors&amp;ust=1714083839910491&amp;usg=AOvVaw2sFHW9wEGwYP7YtU3Wd_ZN'
            >
              Bus tub specs
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref15' id='ftnt15'>
            [15]
          </a>
          <span className='c10'>&nbsp; </span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.webstaurantstore.com/noble-products-full-size-all-purpose-peg-rack-with-open-sides/274RKOPAPPEG.html?utm_source%3Dgoogle%26utm_medium%3Dcpc%26utm_campaign%3DGoogleShopping%26gclid%3DCj0KCQiAnrOtBhDIARIsAFsSe51kbMXNAWYrNm58HEMtUdDg1qyw5TN7sF6Z5CHaGEHWbZoh6VrXl9MaAsYfEALw_wcB&amp;sa=D&amp;source=editors&amp;ust=1714083839911192&amp;usg=AOvVaw3NKU85bWy5TjE8KYRFT7GD'
            >
              Dishwashing rack measurement source{' '}
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref16' id='ftnt16'>
            [16]
          </a>
          <span className='c10'>&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.webstaurantstore.com/regency-24-x-60-green-epoxy-drying-rack-shelf-kit-with-64-posts-and-casters-3-slots/460EG2460D3C.html?utm_source%3Dgoogle%26utm_medium%3Dcpc%26utm_campaign%3DGoogleShopping%26gclid%3DCj0KCQiAqsitBhDlARIsAGMR1RjoUuO-8XeQR29GsJJfRKcP9hDAWE8HyUdTPudrpNdeM7-aQVINTPoaArfxEALw_wcB&amp;sa=D&amp;source=editors&amp;ust=1714083839911979&amp;usg=AOvVaw208T7VD8DhzPNWfiwBHAZL'
            >
              Commercial drying rack specs{' '}
            </a>
          </span>
        </p>
      </div>
      <div>
        <p className='c9'>
          <a href='#ftnt_ref17' id='ftnt17'>
            [17]
          </a>
          <span className='c10'>&nbsp;</span>
          <span className='c0'>
            <a
              className='c3'
              href='https://www.google.com/url?q=https://www.cudakitchen.com/blakeslee-r-m-50-single-wash-rack-conveyor-type-dishwasher-28-5-gallons-198-racks-per-hour?gad_source%3D1%26gclid%3DCj0KCQjw2PSvBhDjARIsAKc2cgNA8-J0bWiw0OdzclFAfXZsEd1lnMeoNxCPTNRzSQTrxQZJpmPDoLMaAtyfEALw_wcB&amp;sa=D&amp;source=editors&amp;ust=1714083839912668&amp;usg=AOvVaw2r-ssixFGAPu9yOTWJPEDV'
            >
              Single-tank conveyor dishwasher
            </a>
          </span>
        </p>
      </div>
    </>
  );
}
