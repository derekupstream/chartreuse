...
import { useGetReusableLineItems } from 'client/projects';
...
export function ReusablePurchasingStep({ isUpstream, readOnly }: { isUpstream: boolean; readOnly: boolean }) {
  ...
  } = useGetReusableLineItems(projectId);
  const { data: reusableProducts, isLoading: isLoadingReusableProducts } = useGetReusableProducts();
  ...
  const items = useMemo(
    () =>
      (lineItems || []).reduce<{
        ...
        const product = reusableProducts?.find(p => p.id === item.productId);
        if (product) {
        ...
        } else if (reusableProducts?.length) {
