...
import { importReusableLineItemsFromExcel } from 'lib/inventory/importReusableLineItemsFromExcel';
import type { ImportedReusableLineItem, ImportResult } from 'lib/inventory/importReusableLineItemsFromExcel';
...
export function ImportButton({ projectId, onImport }: Props) {
  ...
  const uploadButtonProps: UploadProps = {
    ...
    beforeUpload: async file => {
      try {
        const result = await importReusableLineItemsFromExcel(file);
  ...
  async function importFile() {
    ...
    setIsUploading(true);
    try {
      await chartreuseClient.bulkImportReusableLineItems(projectId, fileState.result.items);
      hideModal();
      message.success(`Successfully imported ${fileState.result.items.length} items`);
      onImport();
    } catch (error) {
      console.error('Error importing reusable line items', error);
  ...
  return (
    <>
      ...
      <Modal
        open={open}
        onCancel={hideModal}
        title='Import Reusable Items from Excel'
        width={600}
        footer={[
      ...
      >
        <div style={{ marginBottom: 16 }}>
          <Text>
            Upload an Excel file with a "userinputs" sheet containing your reusable item data. The file should have
